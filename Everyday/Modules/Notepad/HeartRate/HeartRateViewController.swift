...
final class HeartRateViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        measureView.layer.addSublayer(trackLayer)
        measureView.layer.addSublayer(shapeLayer)
    }
    ...
    private func setupLoadIndicator() {
        ...
        let circularPath = UIBezierPath(arcCenter: .zero, radius: Constants.LoadIndicator.radius,
        ...
        progressLabel.center = CGPoint(x: center.x, y: view.center.y + Constants.LoadIndicator.centerOffset / 2 + Constants.LoadIndicator.size.height / 2)
    }
    ...
    private func layout() {
        ...
        let center = CGPoint(x: view.bounds.midX, y: view.bounds.midY + 120)
    ...
    private func updateProgressLabel() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self?.progressLabel.attributedText = NSAttributedString(string: "\(Int(percentage * 100))%", attributes: Constants.Styles.progressAttributes)
        }
...
private extension HeartRateViewController {
    struct Constants {
        ...
        struct LoadIndicator {
            ...
            static let duration: CFTimeInterval = 70
            static let centerOffset: CGFloat = 180
